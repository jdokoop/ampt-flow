...
c If dealing with He3, get nucleon positions from realistic description of the wavefunction
        if(IHNT2(1).EQ.3) then
        X1=0
        Y1=0
        Z1=0
        X2=0
        ...
        Y3=0
        Z3=0
        CALL HE3WF(X1,Y1,Z1,X2,Y2,Z2,X3,Y3,Z3)
...
C
        ...
        DO 70 JT=1,IHNT2(3)
           ...
           R2=(42*0.1)/HIPR1(40)
C                ********A collision occurs if B2 <= R2
C                ********JDOK - May 2015: Modify initial geometry determination to match PHOBOS Glauber MC
...
C j.nagle - 11/14/2010 - could add print out here on YP(1-3,JP) and YT(1-3,JT) for determining eccentricity etc.....
C only if event has at least one NCOLL, otherwise it will terminate and run again....
C JDOK - May 2015: offset projectile coordinates by impact paramters so that projectile and target nucleons can be treated in the same reference frame
       WRITE (77, *) NCOLT
       DO 999 NC=1,NCOLT
          XPO=YP(1,IPCOL(NC))+BBX
          YPO=YP(2,IPCOL(NC))+BBY
          WRITE(77, *) XPO, YPO, YP(3,IPCOL(NC))
          WRITE(77, *) YT(1,ITCOL(NC)), YT(2,ITCOL(NC)), YT(3,ITCOL(NC))
 999   CONTINUE
...
c     &        XMASS5(I), E5(I)
           WRITE (14, 512) PX5(I), PY5(I), PZ5(I), ITYP5(I),
     &        XMASS5(I), E5(I),LSTRG1(I), LPART1(I), FT5(I)
...
c       Subroutine to compute nucleon positions in 3He from a realistic desciption of the wavefunction
c       Output: Xi,Yi,Zi cartesian coordinates of i-th nucleon
        SUBROUTINE HE3WF(X1, Y1, Z1, X2, Y2, Z2, X3, Y3, Z3)
        REAL X1,Y1,Z1,X2,Y2,Z2,X3,Y3,Z3,FOO1,FOO2,FOO3,FOO4,U
        
c       Open 3He wavefunction file with 6K events
        OPEN(UNIT = 32,FILE = "he3_plaintext.dat")
        
c       Generate random number
        CALL RANDOM_NUMBER(U)

c       Get positions from file 
        DO 10 J = 1, FLOOR(U*6000+1), 1
           READ(32,*) X1,Y1,Z1,X2,Y2,Z2,X3,Y3,Z3,FOO1,FOO2,FOO3,FOO4
 10     CONTINUE
